...
def init_callbacks(dash_app,engine):
	...
	def update_select(plane,trigger_record,raw_data_file):
		...
		try: 
			channel_num=[]
			if "Z" in plane:
				channel_num.extend([{'label':str(n), 'value':(n)} for n in (trigger_record_data(engine,trigger_record,raw_data_file).df_Z.columns)])
			if "V" in plane:
				channel_num.extend( [{'label':str(n), 'value':(n)} for n in (trigger_record_data(engine,trigger_record,raw_data_file).df_V.columns)])
			if "U" in plane:
				channel_num.extend([{'label':str(n), 'value':(n)} for n in (trigger_record_data(engine,trigger_record,raw_data_file).df_U.columns)])
			return(channel_num)
